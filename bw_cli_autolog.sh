...
# Variables à définir par l'utilisateur.
Bitwarden_Email="<EMAIL_DE_CONNEXION_A_BITWARDEN>"
Bitwarden_Server="<URL_DE_L_INSTANCE_BITWARDEN>"

# Variables internes au script
Inception_Detect=0 # Détection de terminaux parents/enfants
Local_Version="2.2.7"

# Fonction de suppression et nettoyage des identités
identities_destroy() {
	tput setaf 4
	...
	grep -in "printf.*\"\%s\".*\"\$BW_List\"" "${HOME}"/.identities-loaded.sh | grep ">" | cut -d ">" -f 2 | sed -e "s,\~,${HOME},g ; s,\"\$HOME\",${HOME},g ; s,^\ ,,g" | while IFS= read -r each; do
		rm -rf "${each}"
	done
	unset BW_Session
...
# Processus de création des identités
